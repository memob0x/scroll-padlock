...
export const getLayout = (element, scroller) => {
    ...
    const outerWidth = outer[LAYOUT_WIDTH];
    const outerHeight = outer[LAYOUT_HEIGHT];

    // Element area without scrollbars
    const innerWidth = element[LAYOUT_WIDTH_CLIENT];
    const innerHeight = element[LAYOUT_HEIGHT_CLIENT];
    ...
    return {
        ...
        [LAYOUT_WIDTH_SCROLL]: element[LAYOUT_WIDTH_SCROLL],
        [LAYOUT_HEIGHT_SCROLL]: element[LAYOUT_HEIGHT_SCROLL],
