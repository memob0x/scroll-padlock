...
export default (element) => ({
  ...
  [TOP]: element?.[SCROLL + Y_UPPERCASE]
  ?? element?.[PAGE + Y_UPPERCASE + OFFSET_CAPITALIZED]
  ?? element?.[SCROLL + TOP_CAPITALIZED]
  ?? 0,
  [LEFT]: element?.[SCROLL + X_UPPERCASE]
  ?? element?.[PAGE + X_UPPERCASE + OFFSET_CAPITALIZED]
  ?? element?.[SCROLL + LEFT_CAPITALIZED]
  ?? 0,
});
