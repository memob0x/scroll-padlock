 ...
 * @param {HTMLElement} target The target, the dom node where
 * the given styler is placed (usually head element)
...
const set = (element, styler, target, layout, scroll) => {
  ...
  const dataAttrValue = element?.getAttribute(DOM_DATA_ATTRIBUTE_NAME)
  ?? getElementParentsLength(element) + HYPHEN + getElementIndex(element);
  ...
  let rules = '';
  for (let i = 0, j = rulesDataKeys.length; i < j; i += 1) {
 ...
 * @param {HTMLStyleElement} styler The styler element
 * to be removed from target element (usually head)
 * @returns {HTMLStyleElement} Styler element
 */
 ...
 * @param {String} method Whether is a registration
 * or an unregistration method (accepts "add" or "remove")
 * @param {HTMLElement} element The given element whose css variables need to be updated
 * @param {HTMLStyleElement} styler The styler element where the css variables are written
 * @param {HTMLElement} target The target, the dom node
 * where the given styler is placed (usually head element)
 * @param {Object} layout The layout dimensions objects to be set in css variables
 * @param {Object} scroll The scroll position to be set in css variables
 * @returns {HTMLStyleElement} Styler element
 */
