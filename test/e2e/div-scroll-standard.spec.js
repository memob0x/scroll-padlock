...
describe('div-scroll-standard', () => {
  ...
  it('should be able to ensure scroll position standard approach', async () => {
    const page = await browseHtmlPlaygroundFile(browser, resolve('./test/e2e/div-scroll-standard.html'));
    ...
    await page.evaluate(() => document.querySelector('#el').scrollTo(0, 9999));

    const firstImage = await takeBrowserScreenshot(page, 'div-scroll-standard-0');

    await page.evaluate(() => window.lock());

    const secondImage = await takeBrowserScreenshot(page, 'div-scroll-standard-1');
    ...
    const thirdImage = await takeBrowserScreenshot(page, 'div-scroll-standard-2');
    ...
    const fourthImage = await takeBrowserScreenshot(page, 'div-scroll-standard-3');
